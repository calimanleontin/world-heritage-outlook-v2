 ...
 *
 * @FacetsUrlProcessor(
 *   id = "iucn_facets_pretty_paths",
 *   label = @Translation("Iucn Pretty paths"),
 *   description = @Translation("Iucn Pretty paths uses slashes as separator and prefixes current language in the url, e.g. /fr/brand/drupal/color/blue"),
 * )
 */
class IucnFacetsPrettyPathsUrlProcessor extends UrlProcessorPluginBase {
  ...
  public function buildUrls(FacetInterface $facet, array $results) {
    ...
    $filters = substr($path, (strlen($this->currentLanguage . $facet->getFacetSource()->getPath())));
    ...
    foreach ($results as &$result) {
      ...
      $url = Url::fromUri('base:' . $this->currentLanguage . $facet->getFacetSource()->getPath() . $filters_current_result);
  ...
  protected function initializeActiveFilters($configuration) {
    ...
    $facet_source_path =  $this->currentLanguage . $facet_source_path;
