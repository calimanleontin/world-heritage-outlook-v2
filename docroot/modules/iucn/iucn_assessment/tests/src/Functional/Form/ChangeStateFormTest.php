...
class ChangeStateFormTest extends WorkflowTestBase  {
  ...
  public function testValidation() {
    ...
    $this->assertSession()->pageTextContains('Global assessment level field is required.');
    $this->assertSession()->pageTextContains('Assessment of Conservation Outlook field is required.');
    $this->assertSession()->pageTextContains('Protection overall rating outside factors field is required.');
    $this->assertSession()->pageTextContains('Overall assessment of protection and management field is required.');
    $this->assertSession()->pageTextContains('Protection overall rating field is required.');
    $this->assertSession()->pageTextContains('Assessment of the effectiveness of protection and management in addressing threats outside the site field is required.');
    ...
    $this->assertSession()->pageTextContains('Overall current threats rating field is required.');
    $this->assertSession()->pageTextContains('Overall Assessment of current Threats field is required.');
    $this->assertSession()->pageTextContains('Overall potential threats rating field is required.');
    $this->assertSession()->pageTextContains('Overall Assessment of Potential Threats field is required.');
    $this->assertSession()->pageTextContains('Overall assessment of threats rating field is required.');
    $this->assertSession()->pageTextContains('Overall Assessment of Threats field is required.');
    $this->assertSession()->pageTextContains('Identifying and describing values field is required.');
    $this->assertSession()->pageTextContains('Assessment field is required.');
    $this->assertSession()->pageTextContains('Assessment of the current state and trend of World Heritage values field is required.');
    $this->assertSession()->pageTextContains('Trend field is required.');

    $this->assertSession()->pageTextContains('Summary of the values - Assessment field is required.');
    $this->assertSession()->pageTextContains('Summary of the values - Justification of assessment field is required.');
    $this->assertSession()->pageTextContains('Summary of the values - Trend field is required.');
    $this->assertSession()->pageTextContains('Summary of benefits field is required.');
    ...
    $this->assertSession()->pageTextNotContains('Summary of the values - Assessment field is required.');
    $this->assertSession()->pageTextNotContains('Summary of the values - Justification of assessment field is required.');
    $this->assertSession()->pageTextNotContains('Summary of the values - Trend field is required.');
    $this->assertSession()->pageTextNotContains('Summary of benefits field is required.');
    ...
    $stateChangeForm = Url::fromRoute('iucn_assessment.node.state_change', ['node' => $assessment->id()]);
    $this->drupalGet($stateChangeForm);
    $this->assertSession()->pageTextContains('Specific benefits, Summary fields are required for all rows in Understanding benefits table.');
    $this->assertSession()->pageTextContains('Description, Organisation fields are required for all rows in Compilation of active conservation projects table.');
    $this->assertSession()->pageTextContains('Justification of assessment, Assessment, Topic fields are required for all rows in Assessing Protection and Management table.');
    $this->assertSession()->pageTextContains('Reference field is required for all rows in References table.');
    $this->assertSession()->pageTextContains('Description, Value fields are required for all rows in Other important biodiversity values table.');
    $this->assertSession()->pageTextContains('Description, WH Criteria, Value fields are required for all rows in Identifying and describing values table.');
    $this->assertSession()->pageTextContains('State, Justification of assessment, Trend fields are required for all rows in Assessing values table.');
    $this->assertSession()->pageTextContains('Justification, Category, Assessment, Specific threat affecting site fields are required for all rows in Current threats table.');
    $this->assertSession()->pageTextContains('Justification, Category, Assessment, Specific threat affecting site fields are required for all rows in Potential threats table.');
    ...
    $this->drupalGet($stateChangeForm);
    $this->assertSession()->pageTextNotContains('Subcategories field is required');
    $this->assertSession()->pageTextNotContains('Specific benefits field is required for all rows in Understanding benefits table.');
    ...
    $this->assertSession()->pageTextContains('Specific benefits field is required for all rows in Understanding benefits table.');
    ...
    foreach (self::REQUIRED_DEPENDENT_FIELDS as $field => $categories) {
      ...
      foreach ($categories['subcategories'] as $category) {
        ...
        if (!$term instanceof TermInterface) {
          $term = TestSupport::createSampleEntity('taxonomy_term', 'assessment_threat', ['name' => $category, 'tid' => $category, 'parent' => $parentCategory]);
        }
