...
class Coordinate extends ProcessPluginBase {
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    if(isset($this->configuration['type']) && $this->configuration['type'] == 'x'){
      $data = json_decode(file_get_contents($url));
      if($data && isset($data->features[0]->centroid->x)){
        return $data->features[0]->centroid->x;
      }
    }

    if(isset($this->configuration['type']) && $this->configuration['type'] == 'y'){
      $data = json_decode(file_get_contents($url));
      if($data && isset($data->features[0]->centroid->y)){
        return $data->features[0]->centroid->y;
      }
    }
    return FALSE;
  }
